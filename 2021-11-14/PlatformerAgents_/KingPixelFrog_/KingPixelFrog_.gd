...
signal on_KingPixelFrog_ready(_self)
...
func _ready() -> void:
    ...
    emit_signal("on_KingPixelFrog_ready", self)
    $anim.play("Idle")
    pass

func connect_recievers():
    for i in get_tree().get_nodes_in_group("KingPixelFrogRecievers"):
        connect("on_KingPixelFrog_ready", i ,"on_KingPixelFrog_ready")
