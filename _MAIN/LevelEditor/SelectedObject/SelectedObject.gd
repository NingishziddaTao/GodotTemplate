...
func _input(event):
    ...
    if event is InputEventMouseButton and event.button_index == BUTTON_LEFT:
        if event.get_shift():
            Player.start_position = get_global_mouse_position()
        ...
        update()# _draw
    elif event is InputEventKey and event.scancode == KEY_X or event is InputEventKey and event.scancode == KEY_ESCAPE:
    ...
    elif event is InputEventKey and event.scancode == KEY_R or event is InputEventKey and event.scancode == KEY_BACKSPACE:
...
func mouse_entered(container):
    ...
    get_texture(container)

func get_texture(container):# Also called in ObjectSelectPanel
    if current_object:
...
func delete_tiles(area:Array):
    if deleting and area and current_object:
        if current_object.get_class() == "TileMap":
...
func delete_node(area:Array):
    var excluded = ["TileMap", "WorldEnvironment", "Node2D", "CanvasModulate"]
    if deleting and area and current_object:
        for node in Scene.get_children():
            if not node.get_class() in excluded and !node.is_in_group("Players"):
                if node.global_position.x > area[0].x and node.global_position.x < area[1].x:
                    if node.global_position.y > area[0].y and node.global_position.y < area[1].y:
                        print('LevelEditor/SelectedObject/SelectedObject.gd: deleted', node.name)
                        node.queue_free()
