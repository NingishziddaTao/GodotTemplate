...
#var tileset = 0
var numpad_keys = [KEY_KP_0, KEY_KP_1, KEY_KP_2, KEY_KP_3, KEY_KP_4, KEY_KP_5, KEY_KP_6, KEY_KP_7, KEY_KP_8, KEY_KP_9]
...
enum {AUTOTILES, STATIC, ANIMATED}
...
func autotile_container_setup():
        ...
        for nr in autotiles.size():
            print('LevelEditor/ObjectSelectPanel/ObjectSelectPanel.gd: ', autotiles[nr])
...
func _input(event):
    if event is InputEventKey:
        if event.scancode in numpad_keys:
            # Select tab Containers
            var n = int(str(event.scancode)[-1])
            match $TabContainer.current_tab:
                AUTOTILES: 
                    SelectedObject.current_object = autotile_containers[n].packed_scene
                    SelectedObject.get_texture(autotile_containers[n])
                STATIC:
                    SelectedObject.current_object = static_containers[n].packed_scene
                    SelectedObject.get_texture(static_containers[n])
                ANIMATED:  
                    SelectedObject.current_object = animated_containers[n].packed_scene
                    SelectedObject.get_texture(animated_containers[n])

        # Select Tabs
        if event.scancode in numpad_keys and event.get_shift() == true:
            var n = int(str(event.scancode)[-1])
            if n < $TabContainer.get_tab_count():
                $TabContainer.current_tab = n

            print('LevelEditor/ObjectSelectPanel/ObjectSelectPanel.gd: ', $TabContainer.current_tab)
        

        #print('LevelEditor/ObjectSelectPanel/ObjectSelectPanel.gd: ', autotile_containers[n].packed_scene)
