...
func _draw() -> void:
    ...
    for i in range(-yWidth, yWidth, steps):
        ...
        y_result = space_state.intersect_ray(global_position+y_ray_position,
                                               y_ray_position+global_position+y_ray_cast_to, [self, Player])
        if y_result:
            y_array.append(y_result)
            draw_line(y_ray_position, y_result.position - position.rotated(-rotation), Color(1, 4, 1, ray_visible))
    ...
    for i in range(-xWidth, xWidth, steps):
        ...
        x_result = space_state.intersect_ray(global_position+x_ray_position,
                                               x_ray_position+global_position+x_ray_cast_to, [self, Player])
        if x_result:
            x.append(x_result)
            draw_line(x_ray_position, x_result.position - position.rotated(-rotation), Color(1, 4, 1, ray_visible))
